...
// Selector resolution + resilient click/fill live in ./selectors.js. Thin
// shims below adapt the new API (which takes an opts object) to the existing
// call-site signatures used throughout this file.
const resolveSelector = _resolveSelector;

async function debugHighlightAndShot(p, box, label) {
  return _debugHighlightAndShot(p, box, label, TEMP_DIR);
}

async function smartClick(p, step, selector, timeout, wss, path) {
  return _smartClick(p, step, selector, timeout, wss, path, { broadcastStatus, tempDir: TEMP_DIR });
}

async function smartFill(p, step, selector, value, timeout, wss, path) {
  return _smartFill(p, step, selector, value, timeout, wss, path, { broadcastStatus });
}
